  ...
  return (
    ...
    >
      <div 
        ...
        style={{ padding: '10px 0', fontWeight: 'bold', display: 'flex', justifyContent: 'space-between' }}
      >
        <span>직군 필터</span>
      ...
      {isFilterOpen && ( 
        <div className="job-filter-content open" style={{ marginTop: '10px' }}>
          {Object.entries(POSITION_CATEGORIES).map(([category, data]) => {
            ...
            const isAllSelected = data.roles.every(r => selectedRoles.includes(r.value));
            ...
            return (
              <div key={categoryKey} className="filter-category" style={{ marginBottom: '15px' }}>
                ...
                {/* 개별 직무 */}
                {data.roles.map(role => (
                  <label key={role.value} style={{ display: 'block', marginLeft: '15px', margin: '3px 0' }}>
                    <input
                      type="checkbox"
